...
@implementation StructureGLString
...
- (void) genTexture; // generates the texture without drawing texture to current context
{
	...
	{
		frameSize.height = heightForStringDrawing([string string], [string attribute:NSFontAttributeName atIndex:0 effectiveRange:NULL], frameSize.width);
	...
	/*if (staticFrame)
	{
		float w = frameSize.width - marginSize.width;
		float h = heightForStringDrawing([string string], [string attribute:NSFontAttributeName atIndex:0 effectiveRange:NULL], w); 
		[string drawInRect:NSMakeRect(marginSize.width, marginSize.height, w, h)];
	}
	else
	{
		[string drawAtPoint:NSMakePoint (marginSize.width, marginSize.height)]; // draw at offset position
	}*/
	[string drawInRect:NSMakeRect(marginSize.width, marginSize.height, frameSize.width - marginSize.width, frameSize.height - marginSize.height)];
